...
from mantenedor.signals import normalizar_campos
from mantenedor.signals import contar_productos
from mantenedor.signals import calcular_dv
from mantenedor.signals import calcular_dv_trabajador
from mantenedor.signals import iniciar_nuevo_inventario
from mantenedor.signals import nuevo_producto_inventario


class Categoria(models.Model):
    """Representa una categoria a la que pertenece un producto"""
    nombre = models.CharField(max_length=200,unique=True,help_text=u'escriba el nombre de la categoría.')
    supercategoria = models.ForeignKey('self',verbose_name = u"Categoría Superior",null=True,blank=True)
...
class Producto(models.Model):
    codigo_barra = models.CharField(max_length=100,unique=True,verbose_name=u"código de barra")
    codigo_manual = models.BooleanField(verbose_name=u"código manual")
    nombre = models.CharField(max_length=200,help_text='Escriba el nombre del producto.')
    marca = models.CharField(max_length=50,help_text='Escriba la marca del producto.')
...
class Bodega(models.Model):
    ...
    venta = models.BooleanField(default=False,help_text=u'Indique si esta bodega corresponde a una \
                                                            bodega de venta.')
    productos = models.ManyToManyField('Producto',through='movimiento.ProductoBodega')
...
class Promocion(models.Model):
    codigo = models.CharField(max_length=100, unique=True, verbose_name=u"código promoción")
...
class Proveedor(models.Model):
    ...
    ingresos = models.ManyToManyField('movimiento.ProductoBodega',through='movimiento.Ingreso')
    def __unicode__(self):
...
class Trabajador(models.Model):
    cedula_identidad = models.PositiveIntegerField(u"Cédula de identidad",unique=True)
    ...
    user = models.ForeignKey(User,verbose_name=u'Usuario',unique=True,null=True,blank=True)
