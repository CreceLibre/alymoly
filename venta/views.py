...
from venta import clases, forms, recursos, excepciones, models
from settings import ROOT
from django.utils.translation import ugettext
from django.template import RequestContext
from settings import NOMBRE_SUCURSAL, DESCRIPCION_PROMOCION, \
...
def autentificacion(a_request):
    if a_request.method == 'POST':
        ...
        if formulario_.is_valid():
            ci_,dv_ = recursos.get_ci_dv(formulario_.cleaned_data['cedula_identidad'])
            ci_ = str(int(ci_)) #problemas con el 0 para las personas menores a 10 millones
            try:
                ...
                return render_to_response('autentificacion.html', {'formulario': formulario_})
            ...
            if user_ is not None and user_.is_active and user_ == trabajador_.get_user():
                ...
                formulario_._errors['__all__'] = [u"Su usuario de sistema no existe o no está activo."]
                return render_to_response('autentificacion.html', {'formulario': formulario_})
...
def turno_abrir(a_request):
    ...
    if a_request.method == "POST":
        ...
        if formulario_.is_valid():
            ...
            try:
                ...
                formulario_._errors['__all__'] = [u"%s  Favor autentificarse nuevamente."%a_mensaje]
    else:
    ...
    return render_to_response('turno/turno.html',
                              {
                               'trabajador':a_request.session['trabajador'],
...
def venta(a_request):
    ...
    if a_request.user.is_authenticated():
        if not a_request.user.is_superuser and not a_request.user.is_staff:
            return render_to_response('venta/index.html',
                                      {'inicio_automatico':a_request.session['inicio_automatico']},
                                      context_instance=RequestContext(a_request))
        else:
            if a_request.is_ajax():
                #mensaje_ = u"No tiene privilegios para realizar esta acción.  Debe iniciar sesion nuevamente."
...
def cuerpo_venta(a_request):
    ...
    return render_to_response('venta/cuerpo.html')
...
def buscar_elemento(a_request, codigo_barra_):
    verificar_turno(a_request)
    try:
        elemento = clases.Producto(verificar_existencia=True,codigo_barra=codigo_barra_,descripcion=DESCRIPCION_PRODUCTO)
        ...
        try:
            ...
            raise excepciones.PromocionError(u"Producto o Promoción asociado al código %s no existe"%(codigo_barra_))
        else:
            data = {'descripcion':elemento.get_descripcion(),'precio_venta':recursos.formato_dinero(elemento.precio_venta)}
            a_request.session['elemento'] = elemento 
...
def agregar_elemento(a_request, cantidad_):
    ...
    linea_detalle = venta.add_linea_detalle(clases.LineaDetalle(especificacion=elemento, cantidad=cantidad))
    ...
    if venta.ultimo_tipo_elemento == 1:
        for elemento_ in elemento.componentes:
            componentes_data.append([elemento_.cantidad_compone,elemento_.get_descripcion()])
...
def aumentar(a_request, codigo_barra_):
    verificar_turno(a_request)
    venta = a_request.session['venta']
    linea_detalle = venta.aumentar_linea_detalle(codigo_barra_.replace('_','*')) # se transforma a codificacion interna
    a_request.session['venta'] = venta
    venta_data = {'total_afecto':recursos.formato_dinero(venta.monto_afecto),
            'total_exento':recursos.formato_dinero(venta.monto_exento),
            'monto_total':recursos.formato_dinero(venta.monto_total),
...
def disminuir(a_request, codigo_barra_):
    verificar_turno(a_request)
    venta = a_request.session['venta']
    linea_detalle = venta.disminuir_linea_detalle(codigo_barra_.replace('_','*'))
    a_request.session['venta'] = venta
    venta_data = {'total_afecto':recursos.formato_dinero(venta.monto_afecto),
            'total_exento':recursos.formato_dinero(venta.monto_exento),
            'monto_total':recursos.formato_dinero(venta.monto_total),
...
def cabecera(a_request):
    verificar_turno(a_request)
    turno = a_request.session['turno']
    fecha = turno.get_fecha_apertura_sistema().strftime("%A-%d-%B-%Y-%H:%M:%S").split('-')
    dia_semana = ugettext('%(day)s'%{'day':fecha[0]})
    mes = ugettext('%(month)s'%{'month':fecha[2]})
    ...
    if a_request.method == "POST":
        ...
        if formulario_.is_valid() and formulario_deposito_.is_valid():
            try:            
                ...
                turno_.cerrar(fecha_cierre_sistema=fecha_, boleta_deposito=formulario_deposito_)
                auth.logout(a_request)
                return HttpResponseRedirect('/venta/turno/cerrado/%s/'%turno_.id)
            except Exception, a_mensaje:
                raise Exception("Problemas para cerrar el turno. %s"%a_mensaje)
    else:
        formulario_ = forms.CerrarTurnoForm()
        formulario_deposito_ = forms.BoletaDepositoForm()
    return render_to_response("turno/cierre_turno.html",
                              {'formulario':formulario_,
...
def confeccionar_boleta(**kwargs):
    boleta_img_ = Image.open(ROOT('media/static/images/venta/boleta_deposito.png'))
    ...
    boleta_draw_.text((MARGEN_DERECHO - font.getsize(texto)[0],MARGEN_SUPERIOR),texto,fill='black',font=font)
    MARGEN_SUPERIOR += 60
...
def boleta_deposito(a_request, a_id):    
    try:
        ...
        if turno_.estado == 1:
            raise excepciones.TurnoError(u"Turno debe estar cerrado para consultar su boleta de deposito.")
    ...
    datos['fecha'] = turno_.fecha_apertura_sistema.strftime("%d/%m/%Y") + ' - ' + turno_.fecha_cierre_sistema.strftime("%d/%m/%Y")
    datos['trabajador'] = turno_.trabajador.get_nombre_completo().upper()
    datos['turno'] = turno_.fecha_apertura_sistema.strftime("%H:%M:%S %p").lower() + ' - ' + turno_.fecha_cierre_sistema.strftime("%H:%M:%S %p").lower()
    ...
    response_ = HttpResponse(mimetype='image/png')
    boleta_src_.save(response_,'PNG')
    return response_   


def turno_cerrado(a_request, a_id):
    return render_to_response("turno/turno_cerrado.html",{'id':a_id})
