...
def criticos(request, bodega=None):
    ...
    return render(request, 'inventario/critico.html', {'form': form, })
...
def actual(request, bodega=None):
    ...
    if bodega:
        ...
        return render(request, 'inventario/parcial/actual.html', {
            'existencias': existencias.select_related(),
        })
    return render(request,'inventario/actual.html', {
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            id_compuestos = Producto.objects.exclude(
                compone=None).values_list('compone_id', flat=True)
            productos = ProductoBodega.objects.filter(
                bodega=bodega).exclude(producto__pk__in=id_compuestos)
            productos.update(existencia=valor)
            messages.add_message(request, messages.INFO, 'El inventario ha sido reiniciado satisfactoriamente.')

    else:
        form = ReiniciarInventarioForm()
    return render(request, 'inventario/reiniciar.html', {'form': form})
